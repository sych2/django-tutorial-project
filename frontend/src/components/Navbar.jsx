...
export default function Navbar() {
  ...
  const linkStyle = { color: '#aaa', textDecoration: 'none', fontSize: '0.85rem', letterSpacing: '0.08em', textTransform: 'uppercase' }
    ...
    <nav style={{position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: 'rgba(10,10,10,0.95)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(232,88,12,0.3)', height: '64px'}}>
      <div style={{maxWidth: '1280px', margin: '0 auto', padding: '0 2rem', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
        ...
        <div style={{display: 'flex', alignItems: 'center', gap: '2rem'}}>
          ...
          <Link to="/products" style={linkStyle}>Equipment</Link>
          <Link to="/cart" style={{...linkStyle, position: 'relative'}}>
            Cart
            {totalItems > 0 && (
              <span style={{position: 'absolute', top: '-8px', right: '-12px', background: '#E8580C', color: '#fff', fontSize: '0.6rem', fontWeight: 700, width: '16px', height: '16px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                {totalItems}
              </span>
            )}
          </Link>
          {user ? (
            ...
            <>
              <Link to="/login" style={{...linkStyle, padding: '0.4rem 1rem', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '2px', color: '#fff'}}>Login</Link>
              <Link to="/register" style={{...linkStyle, padding: '0.4rem 1rem', background: '#E8580C', borderRadius: '2px', color: '#fff'}}>Register</Link>
            </>
          )}
        </div>
      </div>
